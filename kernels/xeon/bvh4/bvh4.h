...
namespace embree
{
  ...
  public:
	...
	prefetchL1(((char*)ptr)+1*64);
      ...
      __forceinline       Node* getNode()       { assert(isNode()); return (      Node*)(ptr & ~deco_mask); }
      __forceinline const Node* getNode() const { assert(isNode()); return (const Node*)(ptr & ~deco_mask); }
