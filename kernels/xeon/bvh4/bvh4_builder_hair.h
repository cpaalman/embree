...
namespace embree
{
  ...
  public:
    ...
    /*! Constructor. */
    BVH4BuilderHair (BVH4* bvh, Scene* scene);
  ...
  private:
    ...
    struct BuildTask
    {
      ...
      __forceinline BuildTask (BVH4::NodeRef* dst, size_t depth, size_t size, bool makeleaf, atomic_set<PrimRefBlock>& prims, const NAABBox3fa& bounds)
    ...
    public:
      ...
      __forceinline float standardSAH() const {
        return BVH4::intCost*countfunc(num0)*embree::area(bounds0.bounds) + BVH4::intCost*countfunc(num1)*embree::area(bounds1.bounds);
      }

      /*! calculates modified surface area heuristic for the split */
      __forceinline float modifiedSAH() const {
        return 
          BVH4::travCostUnaligned*countfunc(num0)*embree::area(bounds0.bounds) + BVH4::intCost*bounds0.bounds.upper.w + 
          BVH4::travCostUnaligned*countfunc(num1)*embree::area(bounds1.bounds) + BVH4::intCost*bounds1.bounds.upper.w;
      }
      
      /*! finds the two hair strands */
      static const StrandSplit find(size_t threadIndex, BVH4BuilderHair* parent, atomic_set<PrimRefBlock>& curves);
      
      /*! splits hair list into the two strands */
      void split(size_t threadIndex, BVH4BuilderHair* parent, 
    ...
    public:
      ...
      static ObjectSplit find(size_t threadIndex, size_t depth, BVH4BuilderHair* parent, atomic_set<PrimRefBlock>& curves, const LinearSpace3fa& space);

      /*! splits hairs into two sets */
      void split(size_t threadIndex, BVH4BuilderHair* parent, atomic_set<PrimRefBlock>& curves, atomic_set<PrimRefBlock>& lprims_o, atomic_set<PrimRefBlock>& rprims_o) const;
    ...
    public:
      ...
      /*! finds the two hair strands */
      static const SpatialSplit find(size_t threadIndex, size_t depth, size_t size, BVH4BuilderHair* parent, atomic_set<PrimRefBlock>& curves, const LinearSpace3fa& space);
    ...
    struct FallBackSplit
    {
      ...
      static FallBackSplit find(size_t threadIndex, BVH4BuilderHair* parent, 
